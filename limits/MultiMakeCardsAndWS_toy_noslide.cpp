...
void MultiMakeCardsAndWS_toy_noslide(){
  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
        ...
        if (year[y] == "2017"){
          file=TFile::Open("/home/tier3/wangzqe/research/darkphoton/CWR_toy_study/output_toy_hist.root");
        }
        else if (year[y] == "2018"){
          file=TFile::Open("/home/tier3/wangzqe/research/darkphoton/CWR_toy_study/output_toyhist_2018_5sigma.root");
        }
	//X-SECTION GRAPH
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
	double mass = -1.;
	double rel_reso=0.026;//temporary
	...
	for(int i=355; i<738; i++){
	  	...
	  	//get the histograms
	        TH1D* catA;
                ...
                if(i<590) catA=(TH1D*)file->Get("massforLimitLow");
                else catA=(TH1D*)file->Get("massforLimitHigh");
                catA->SetDirectory(0);
		...
		double massBinWidth = massHigh-massLow;
                ...
                double Low_tem  =  dummy->GetXaxis()->GetXmin();
                double High_tem =  dummy->GetXaxis()->GetXmax();
	  	mass = 0.5*(Low_tem+High_tem);
		...
		RooDataHist data_obs("data_obs", "", RooArgList(*m2mu), catA);
                ...
                TString toyfile,toyname;

                if(i<590){
                   toyfile="ws_toy_low.root";
                }
                else{
                   toyfile="ws_toy_high.root";
                }
                TFile* toy_ws = TFile::Open("/home/tier3/wangzqe/research/darkphoton/CWR_toy_study/"+toyfile, "READ");
                RooWorkspace *w_toy = (RooWorkspace*)toy_ws->Get("workspace");
                RooHistPdf* bkg_pdf = (RooHistPdf*)w_toy->pdf("bkg_pdf");

                RooRealVar *x2 = (RooRealVar*)w_toy->var("m2mu");
                x2->setMax(massHigh);
                x2->setMin(massLow);
                
                bkg_pdf->fitTo(data_obs);
		...
		newcardShape << Form("shapes data_obs	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:data_obs\n",i);
                newcardShape << Form("shapes bkg_mass CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:bkg_pdf\n",i);
		...
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                ...
                newcardShape << Form("res_rel_generic param 0.026 0.0052\n");
		newcardShape << Form("");
