...
void MultiMakeCardsAndWS_ptcut_edgeCWR5sigma(){
  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
	...
	//if (year == "2017") file=TFile::Open("/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/2017/ScoutingRunD/mergedHistos_v1.root");
        if (year[y] == "2017"){
          file2=TFile::Open("~/dphist/CWR/5sigma/ptcut35/mergedHistos_jpsi0p015_2017.root"); 
          file=TFile::Open("~/dphist/CWR/5sigma/ptcut20/mergedHistos_jpsi0p015_2017.root");
        }
        else if (year[y] == "2018"){
          file2=TFile::Open("~/dphist/CWR/5sigma/ptcut35/mergedHistos_jpsi0p015_2018.root"); //61.3 fb -1
          file=TFile::Open("~/dphist/CWR/5sigma/ptcut20/mergedHistos_jpsi0p015_2018.root");
        }
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
	...
	for(int i=355; i<738; i++){
		...
		if ((mass >= 2.61) && (mass <= 4.16)) continue;
...
//                if (mass < 3.0) effcuts = 0.03;
	...
	*/	
		...
		dpworkspace.writeToFile(Form("output_5sigma/dpWorkspace"+year[y]+suff+"_%d.root",i));

		//write the datacard
		char inputShape[200];
		sprintf(inputShape,"output_5sigma/dpCard_"+year[y]+suff+"_m%.3f_%d.txt",mass,i);
