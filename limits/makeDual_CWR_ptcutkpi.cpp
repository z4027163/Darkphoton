...
void makeDual_CWR_ptcutkpi(){

  TString year[2] = {"2017","2018"};
  float norm_dual[2] = {4815.7,14754.5};
  float norm2_dual[2]={5022.8,15410.5};
  float norm03_dual[2]={250,250};
  float norm3_dual[2]={277,277};  //old ratio was nor3/norm03=0.9
  float norm_bkg_control[2]={171684,184589};
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes dkpi_mass highip dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:Gb1\n",i);
		...
		newcardShape << Form("rate    		%f  		%f	   %f         %f        %f    %f \n",
				     effcuts*triggereff*luminosity*rescale[kk], catA->Integral(),norm03_dual[y],effcuts*triggereff*luminosity*rescale[kk]*0.017,norm_bkg_control[y],norm3_dual[y]);
                if (year[y] == "2017"){
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("dkpi_norm_2017 rateParam * dkpi_mass 1.0 [0.01,5]");
		}
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("dkpi_norm_2018 rateParam * dkpi_mass 1.0 [0.01,5]");
		}
