...
void MultiMakeCardsAndWS_ptcut_edgeCWR(){
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
	...
	upsilon->var("res_rel_generic")->setConstant(true);
        ...
        // 58,352 pase 0.65%
        // 115,708 pase 0.325%
	for(int i=355; i<617; i++){
                int ihist=0;
                if(!((i>=355&&i<359)||i==515||(i>=610&&i<=616))) continue;              

                if(i>=355 && i<359) ihist=359;
                if(i==515) ihist=514;
                if(i>=610 && i<=616) ihist=617;
                double pvd_scale=1;
                //if(i!=225) continue;
	  	//get the histograms
	        TH1D* catA;
	        TH1D* catB;
                if (i > 590){
                  catA=(TH1D*)file->Get(Form("massforLimit_CatA%d",ihist));
                  catB=(TH1D*)file->Get(Form("massforLimit_CatB%d",ihist));
                }
                else{
                  catA=(TH1D*)file2->Get(Form("massforLimit_CatA%d",ihist));
                  catB=(TH1D*)file2->Get(Form("massforLimit_CatB%d",ihist));
                }
                catA->SetDirectory(0);
                catB->SetDirectory(0);
		...
		TH1D* catAMVA;
		  ...
		  catAMVA=(TH1D*)IDfileMVA2->Get(Form("massforLimit_CatA%d",imap));
                ...
                catAMVA->SetDirectory(0);
                catBMVA->SetDirectory(0);
	  	catAMVA->Add(catBMVA);
		catAMVA->Rebin(2);
	  	delete catBMVA;
		double countMVA = catAMVA->Integral();
	  	...
	  	TH1D* catBNO=(TH1D*)IDfileNO->Get(Form("massforLimit_CatB%d",imap));
                catANO->SetDirectory(0);
                catBNO->SetDirectory(0);
	  	catANO->Add(catBNO);
		catANO->Rebin(2);
	  	delete catBNO;
		double countNO = catANO->Integral();
                ...
                TH1D* dummy=(TH1D*)file->Get(Form("massforLimit_CatA%d",i));
	  	double massLow  =  dummy->GetXaxis()->GetXmin();
		double massHigh =  dummy->GetXaxis()->GetXmax();
		...
		if (mass >= 8.265) continue;
                if(mass>7.9) continue;
		if ((mass >= 2.59) && (mass <= 4.19)) continue;
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		dpworkspace.writeToFile(Form("output_edge/dpWorkspace"+year[y]+suff+"_%d.root",i));

		//write the datacard
		char inputShape[200];
		sprintf(inputShape,"output_edge/dpCard_"+year[y]+suff+"_m%.3f_%d.txt",mass,i);
