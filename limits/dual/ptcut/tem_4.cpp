  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
	...
	//if (year == "2017") file=TFile::Open("/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/2017/ScoutingRunD/mergedHistos_v1.root");
        if (year[y] == "2017"){
          ...
          file2=TFile::Open("~/dphist/ptcut/IP3p5to11/mergedHistos_jpsi0p015_2017.root");
        }
        else if (year[y] == "2018"){
          file=TFile::Open("~/dphist/sigma_p013/mergedHistos_mva_2018.root"); //61.3 fb -1
          file2=TFile::Open("~/dphist/ptcut/IP3p5to11/mergedHistos_jpsi0p015_2018.root");
        }
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		dpworkspace.writeToFile(Form("output_test/dpWorkspace"+year[y]+suff+"_%d_dual2.root",i));

		//write the datacard
		char inputShape[200];
		sprintf(inputShape,"output_test/dpCard_"+year[y]+suff+"_m%.3f_%d_dual2.txt",mass,i);
