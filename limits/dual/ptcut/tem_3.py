...
os.system("combine -M MultiDimFit -d output_test/dpCard_"+year+"IterV3_m"+file+".txt --algo none --setParameters r=0.0 --setParameterRanges r=0.1,5 --cminDefaultMinimizerStrategy 0  -n SB -m "+mass+" --X-rtd REMOVE_CONSTANT_ZERO_POINT=1 --X-rtd MINIMIZER_freezeDisassociatedParams --cminRunAllDiscreteCombinations --cminDefaultMinimizerTolerance=0.001 --saveWorkspace")
os.system("combine -M MultiDimFit -d output_test/dpCard_"+year+"IterV3_m"+file+".txt --algo none --setParameters r=0.0 --setParameterRanges r=0.001,0.002 --cminDefaultMinimizerStrategy 0  -n Bonly -m "+mass+" --X-rtd REMOVE_CONSTANT_ZERO_POINT=1 --X-rtd MINIMIZER_freezeDisassociatedParams --cminRunAllDiscreteCombinations --cminDefaultMinimizerTolerance=0.001 --saveWorkspace")
...
bkgpdf2 = w.pdf("shapeBkg_dkpi_mass_CatAB")
ndkpi = w.function("n_exp_binCatAB_proc_dkpi_mass").getVal()


print "nkpi_catAB=", ndkpi
...
frac2=w.function("n_exp_binCatAB_proc_dkpi_mass")
#bkgpdf2.plotOn(frame2, RooFit.LineColor(8), RooFit.Name("new_pdf"),RooFit.Normalization(ndkpi,RooAbsReal.NumEvent))
addpeak = RooAddPdf("peak", "sig+dkpi", RooArgList(sigpdf,bkgpdf2),  RooArgList(frac1,frac2))
addpeak.plotOn(frame2, RooFit.LineColor(kOrange+1), RooFit.Name("sig_pdf"), RooFit.Normalization(nsig+ndkpi,RooAbsReal.NumEvent))
bkgpdf2.plotOn(frame2, RooFit.LineColor(8), RooFit.Name("dkpi_pdf"),RooFit.Normalization(ndkpi,RooAbsReal.NumEvent));
...
print "nsig_catAB=",nsig
print "nkpi_highip=",w.function("n_exp_binhighip_proc_dkpi_mass").getVal()
print "nkpi_catAB=",ndkpi
...
latex4.DrawLatex(0.45,0.87, h_string4);
