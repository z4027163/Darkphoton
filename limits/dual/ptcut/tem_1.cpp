...
void tem_1(){
  ...
  int norm2_dual[2]={5400,9000};
  int norm_bkg_control[2]={118981,128700};
//to add, control norm
...
//        { int N=effgraph->GetN(); int i=0; while(i<N) {effgraph->SetPointY(i,effgraph->GetPintY(i)*0.05)} }
	...
	effgraph->SaveAs("output_test/effgraph.root");
	accgraph->SaveAs("output_test/accF.root");
	xsecgraph->SaveAs("output_test/xsecgraph.root");
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		dpworkspace.writeToFile(Form("output_test/dpWorkspace"+year[y]+suff+"_%d.root",i));

		//write the datacard
		char inputShape[200];
		sprintf(inputShape,"output_test/dpCard_"+year[y]+suff+"_m%.3f_%d.txt",mass,i);
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes data_obs highip dpWorkspace"+year[y]+"IterV3_207_dual2.root dpworkspace:data_obs\n");
                newcardShape << Form("shapes bkg_mass highip dpWorkspace"+year[y]+"IterV3_207_dual2.root dpworkspace:bkg_dual_model_"+year[y]+"\n");
                newcardShape << Form("shapes dkk_mass highip dpWorkspace"+year[y]+"IterV3_207_dual2.root dpworkspace:voig\n");
                newcardShape << Form("shapes dkk_mass CatAB dpWorkspace"+year[y]+"IterV3_207.root dpworkspace:voig\n"); 
		...
		newcardShape << Form("rate    		%f  		%f	   %d         %f       %d       %d\n",
				     effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk], catA->Integral(),norm_dual[y],effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk]*0.017,norm_bkg_control[y],norm2_dual[y]);
