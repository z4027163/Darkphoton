...
void MultiMakeCardsAndWS_peak_upsilon(){
	...
	for(int i=369; i<375; i++){
		...
		//compute mass point and define ROOFit variables
	  	bool dontfit=false;
                ...
                TFile* peak_ws = TFile::Open("shape_peaks/ws_upsilon_"+year[y]+".root", "READ");
                ...
                w_peak->var("mean1")->setConstant(true);
                w_peak->var("mean2")->setConstant(true);
                w_peak->var("mean3")->setConstant(true);
                w_peak->var("n")->setConstant(true);
                w_peak->var("res_rel1")->setConstant(true);
                w_peak->var("res_rel2")->setConstant(true);
                w_peak->var("res_rel3")->setConstant(true);
                w_peak->var("peak_ratio1")->setConstant(true);
                w_peak->var("peak_ratio2")->setConstant(true);
                RooRealVar *n_exp_peak = (RooRealVar*)w_peak->var("nsig");
                RooRealVar *n_frac1 = (RooRealVar*)w_peak->var("peak_ratio1");
                //RooAddPdf *peakModel = (RooAddPdf*)w_peak->pdf("sig1");
                RooCBShape *peakModel = (RooCBShape*)w_peak->pdf("sig_CB1"); 
                ...
                double norm_peak=(intX->getVal())*(n_exp_peak->getVal())*(n_frac1->getVal());
                cout << "norm_peak=" << norm_peak << endl;
                double frac_peak = norm_peak/catA->Integral();
                cout << "frac_peak=" << frac_peak << endl;
                RooRealVar frac_peak_fit("frac_peak","frac_peak",frac_peak,0,9*frac_peak);  
