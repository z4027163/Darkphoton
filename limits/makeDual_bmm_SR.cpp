...
void makeDual_bmm_SR(){

  TString year[2] = {"2017","2018"};
  int norm_dual[2] = {210,210};
  int norm2_dual[2]={2000,2000};
  int norm_bkg_control[2]={5705233,7142925};
//to add, control norm
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
        TFile* bmm_ws = TFile::Open("bmm_ws_CR.root", "READ");
        RooWorkspace *w_bmm = (RooWorkspace*)bmm_ws->Get("dpworkspace");
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes bmm_mass CR dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:bmm\n",i);
                newcardShape << Form("shapes bmm_mass CatAB dpWorkspace"+year[y]+"IterV3_%d.root dpworkspace:bmm\n",i); 
		newcardShape << Form("bin		CatAB        CR\n");
		newcardShape << Form("observation 	-1.0         -1.0\n");
		newcardShape << Form("bin     		CatAB		CatAB		CatAB	      	CR		CR\n");
		newcardShape << Form("process 		signalModel_generic     bkg_mass      bmm_mass     bkg_mass    bmm_mass\n");
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("bmm_norm_2017 rateParam * bmm_mass 1.0 [0.2,5]");
		}
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                  newcardShape << Form("bkg_norm2_2018 rateParam CR bkg_mass 1.0\n");
                  newcardShape << Form("bmm_norm_2018 rateParam * bmm_mass 1.0 [0.2,5]");
		}
