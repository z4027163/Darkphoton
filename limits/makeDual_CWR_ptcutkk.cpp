...
void makeDual_CWR_ptcutkk(){

  TString year[2] = {"2017","2018"};
  int norm_dual[2] = {100,111};
  int norm2_dual[2]={90,100};
  int norm_bkg_control[2]={173024,187480};
//to add, control norm
  ...
  //INPUT FILE WITH HISTOGRAMS TO FIT BACKGROUND
	...
	//if (year == "2017") file=TFile::Open("/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/2017/ScoutingRunD/mergedHistos_v1.root");
        if (year[y] == "2017"){
          file2=TFile::Open("~/dphist/CWR/8sigma/ptcut35/mergedHistos_jpsi0p015_2017.root");
          file=TFile::Open("~/dphist/CWR/8sigma/ptcut20/mergedHistos_jpsi0p015_2017.root");
        }
        else if (year[y] == "2018"){
          file2=TFile::Open("~/dphist/CWR/8sigma/ptcut35/mergedHistos_jpsi0p015_2018.root"); //61.3 fb -1
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
	...
	for(int i=415; i<416; i++){
		...
		TH1D* catAMVA;
                ...
                //int imap=round(139.3214+i*0.3261);   //pase 0.325%
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes dkk_mass highip dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:voig\n",i);
                newcardShape << Form("shapes dkk_mass CatAB dpWorkspace"+year[y]+"IterV3_%d.root dpworkspace:voig\n",i); 
		...
		newcardShape << Form("process 		signalModel_generic     bkg_mass      dkk_mass      signalModel_generic     bkg_mass    dkk_mass\n");
		newcardShape << Form("process 		0    		1           2       0             1              2\n");
		newcardShape << Form("rate    		%f  		%f	   %d         %f       %d       %d\n",
				     effcuts*triggereff*luminosity*rescale[kk], catA->Integral(),norm_dual[y],effcuts*triggereff*luminosity*rescale[kk]*0.017,norm_bkg_control[y],norm2_dual[y]);
                if (year[y] == "2017"){
                  newcardShape << Form("lumi13TeV_2017 lnN      1.020   -   -   -    -   -\n");
                  newcardShape << Form("lumi13TeV_corr lnN      1.011   -   -   -    -   -\n");
                  newcardShape << Form("sig_shape_2017 lnN      1.017   -   -   -    -   -\n");
                }
                if (year[y] == "2018"){
                  newcardShape << Form("lumi13TeV_2018 lnN      1.015   -   -   -    -   -\n");
                  newcardShape << Form("lumi13TeV_corr lnN      1.020   -   -   -    -   -\n");
                  newcardShape << Form("sig_shape_2018 lnN      1.014   -   -   -    -   -\n");
                }
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("dkk_norm_2017 rateParam * dkk_mass 1.0 [0.2,5]");
		}
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("dkk_norm_2018 rateParam * dkk_mass 1.0 [0.0,5]");
		}
