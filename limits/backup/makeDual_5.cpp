...
void makeDual_5(){
  ...
  int norm_dual[2] = {6000,10000};
  int norm2_dual[2]={6384,10640};
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
        TFile* dmm_ws = TFile::Open("dmm_ws.root", "READ");
        RooWorkspace *w_mm = (RooWorkspace*)dmm_ws->Get("dpworkspace");
        w_mm->var("dmmmean")->setConstant(true);//all this should actually be automatic. Check!!
        w_mm->var("dmmwidth")->setConstant(true);
        w_mm->var("dmmsigma")->setConstant(true);
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes data_obs highip dpWorkspace"+year[y]+"IterV3_223_dual2.root dpworkspace:data_obs\n");
                newcardShape << Form("shapes bkg_mass highip dpWorkspace"+year[y]+"IterV3_223_dual2.root dpworkspace:bkg_dual_model_"+year[y]+"\n");
                newcardShape << Form("shapes dmm_mass highip dpWorkspace"+year[y]+"IterV3_223_dual2.root dpworkspace:voig_mm\n");
                newcardShape << Form("shapes dmm_mass CatAB dpWorkspace"+year[y]+"IterV3_223.root dpworkspace:voig_mm\n"); 
		newcardShape << Form("bin		CatAB        highip\n");
		newcardShape << Form("observation 	-1.0         -1.0\n");
		newcardShape << Form("bin     		CatAB		CatAB		CatAB	      highip       	highip		highip\n");
		newcardShape << Form("process 		signalModel_generic     bkg_mass      dmm_mass      signalModel_generic     bkg_mass    dmm_mass\n");
		newcardShape << Form("process 		0    		1           2       0             1              2\n");
		newcardShape << Form("rate    		%f  		%f	   %d         %f      8053259.000000    %d\n",
				     effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk], catA->Integral(),norm_dual[y],effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk]*0.017,norm2_dual[y]);
		...
		newcardShape << Form("eff_trig_2018 lnN         %f   -    -   -    -    -\n", triggSys);
                ...
                newcardShape << Form("dmm_ratio lnN            -    -   1.20  -   -    -\n");
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
                  ...
                  newcardShape << Form("dmm_norm_2017 rateParam * dmm_mass 1.0 [0.2,5]");
		}
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                  newcardShape << Form("bkg_norm2_2018 rateParam highip bkg_mass 1.0\n");
                  newcardShape << Form("dmm_norm_2018 rateParam * dmm_mass 1.0 [0.2,5]");
		}
