...
*/
  ...
  float norm_dual[2] = {4997.4,14839.4};    //dtokk SR
  float norm2_dual[2]={5211.5,15433.8};      //dtokk CR
  float norm03_dual[2]={23054.3,24592.9};    //dtokpi SR
  float norm3_dual[2]={32554.6,36055.9};     //dtokpi CR
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
        ...
        w_kpi->var("dkpiwidth")->setConstant(true);
        w_kpi->var("dkpisigma")->setConstant(true);
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		dpworkspace.import(*signalModel);
                ...
                RooVoigtian* dkpiModel = (RooVoigtian*)w_kpi->pdf("voig_kpi");
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                newcardShape << Form("shapes signalModel_generic        highip dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                newcardShape << Form("shapes data_obs highip dpWorkspace"+year[y]+suff+"_%d_dual2.root dpworkspace:data_obs\n",i);
                newcardShape << Form("shapes bkg_mass highip dpWorkspace"+year[y]+suff+"_%d_dual2.root dpworkspace:bkg_dual_model_"+year[y]+"\n",i);
                newcardShape << Form("shapes dkk_mass highip dpWorkspace"+year[y]+suff+"_%d_dual2.root dpworkspace:voig\n",i);
                newcardShape << Form("shapes dkk_mass CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:voig\n",i);
                newcardShape << Form("shapes dkpi_mass highip dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:voig_kpi\n",i);
                newcardShape << Form("shapes dkpi_mass CatAB dpWorkspace"+year[y]+"IterV3_%d.root dpworkspace:voig_kpi\n",i);

		newcardShape << Form("bin		CatAB        highip\n");
		newcardShape << Form("observation 	-1.0         -1.0\n");
		newcardShape << Form("bin     		CatAB		CatAB		CatAB	   CatAB     	highip		highip       highip       highip\n");
		newcardShape << Form("process 		signalModel_generic     bkg_mass      dkk_mass      dkpi_mass      signalModel_generic       bkg_mass    dkk_mass     dkpi_mass\n");
		newcardShape << Form("process 		0    		1           2           3       0         1              2          3\n");
		newcardShape << Form("rate    		%f  		%f	  %f      %f     %f     7119753.000000    %f       %f\n",
				     effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk], catA->Integral(),norm_dual[y],norm03_dual[y],effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk]*0.017,norm2_dual[y],norm3_dual[y]);
		newcardShape << Form("lumi13TeV_"+year[y]+" lnN 	1.026 	-     -   -     -   -    -   -\n");
		...
		newcardShape << Form("bkg_norm_"+year[y]+" rateParam CatAB bkg_mass 1.0\n");
                newcardShape << Form("bkg_norm2_"+year[y]+" rateParam highip bkg_mass 1.0\n");
