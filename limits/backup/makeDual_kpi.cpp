...
/*
  float norm_dual[2] = {4815.7,14754.5};
  float norm2_dual[2]={5022.8,15410.5};
  float norm03_dual[2]={22182.1,15528.3};
  float norm3_dual[2]={23191.1,16248.1};
*/
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                ...
                newcardShape << Form("shapes data_obs highip dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:data_obs\n",i);
                newcardShape << Form("shapes bkg_mass highip dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:bkg_dual_model_"+year[y]+"\n",i);
		...
		newcardShape << Form("process 		signalModel_generic     bkg_mass      dkpi_mass      signalModel_generic     bkg_mass    dkpi_mass\n");
		newcardShape << Form("process 		0    		1           2       0             1              2\n");
		newcardShape << Form("rate    		%f  		%f	   %f         %f        7119753.000000    %f\n",
				     effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk], catA->Integral(),norm03_dual[y],effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk]*0.017,norm3_dual[y]);
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
               ...
               //   newcardShape << Form("dkpi_norm_2017 rateParam * dkpi_mass 1.0 [0.2,5]");
		}
		if (year[y] == "2018"){
