...
void makeDual_CWR_ptcutkk_offpeak(){

  TString year[2] = {"2017","2018"};
  int norm03_dual[2]={20,242};  //dtokk SR
//to add, control norm
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		newcardShape << Form("process 		signalModel_generic     bkg_mass      dkk_mass \n");
		newcardShape << Form("process 		0    		1           2    \n");
		newcardShape << Form("rate    		%f  		%f	   %f\n",
				     effcuts*triggereff*luminosity*rescale[kk], catA->Integral(),norm03_dual[y]);
                if (year[y] == "2017"){
