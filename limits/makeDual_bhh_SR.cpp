...
void makeDual_bhh_SR(){

  TString year[2] = {"2017","2018"};
  int norm_dual[2] = {265,265};
  int norm2_dual[2]={5000,5000};
  int norm_bkg_control[2]={5705233,7131060};
//to add, control norm
...
//        { int N=effgraph->GetN(); int i=0; while(i<N) {effgraph->SetPointY(i,effgraph->GetPintY(i)*0.05)} }
	...
	effgraph->SaveAs("output_highpvd/effgraph.root");
	accgraph->SaveAs("output_highpvd/accF.root");
	xsecgraph->SaveAs("output_highpvd/xsecgraph.root");
...
//        IDfileMVA2=TFile::Open("~/dphist/reverse/lowDY/lowDY_mvajpsi.root");
   ...
   //LOOP OVER MASS INDICES AND MAKE THE CARDS/WORKSPACES
        TFile* bhh_ws = TFile::Open("bhh_ws_CR.root", "READ");
        RooWorkspace *w_bhh = (RooWorkspace*)bhh_ws->Get("dpworkspace");
...
//                if (mass < 3.0) effcuts = 0.03;
		...
		bkg_model_2017.setCorrectionFactor(0.5);
                ...
                //save dtokk peak
                RooHistPdf* bhhModel = (RooHistPdf*)w_bhh->pdf("bhh");
 
		//save into ROO workspace
		RooWorkspace dpworkspace("dpworkspace", "");
		dpworkspace.import(data_obs);
		dpworkspace.import(*signalModel);
                dpworkspace.import(*bhhModel);
		if (year[y] == "2017"){
		  dpworkspace.import(bkg_model_2017);
		}else if (year[y] == "2018"){
		  dpworkspace.import(bkg_model_2018); 
		}
		dpworkspace.writeToFile(Form("output_highpvd/dpWorkspace"+year[y]+suff+"_%d.root",i));

		//write the datacard
		char inputShape[200];
		sprintf(inputShape,"output_highpvd/dpCard_"+year[y]+suff+"_m%.3f_%d.txt",mass,i);
		...
		newcardShape << Form("shapes signalModel_generic	CatAB dpWorkspace"+year[y]+suff+"_%d.root dpworkspace:signalModel_generic\n",i);
                newcardShape << Form("shapes data_obs CR dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:data_obs\n",i);
                newcardShape << Form("shapes bkg_mass CR dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:bkg_dual_model_"+year[y]+"\n",i);
                newcardShape << Form("shapes bhh_mass CR dpWorkspace"+year[y]+"IterV3_%d_dual2.root dpworkspace:bhh\n",i);
                newcardShape << Form("shapes bhh_mass CatAB dpWorkspace"+year[y]+"IterV3_%d.root dpworkspace:bhh\n",i); 
		newcardShape << Form("bin		CatAB        CR\n");
		newcardShape << Form("observation 	-1.0         -1.0\n");
		newcardShape << Form("bin     		CatAB		CatAB		CatAB	      	CR		CR\n");
		newcardShape << Form("process 		signalModel_generic     bkg_mass      bhh_mass     bkg_mass    bhh_mass\n");
		newcardShape << Form("process 		0    		1           2          1              2\n");
		newcardShape << Form("rate    		%f  		%f	   %d          %d       %d\n",
				     effcuts*effgraph->Eval(mass,0,"S")*luminosity*rescale[kk], catA->Integral(),norm_dual[y],norm_bkg_control[y],norm2_dual[y]);
		...
		if (year[y] == "2017"){
		  newcardShape << Form("bkg_norm_2017 rateParam CatAB bkg_mass 1.0\n");
                  newcardShape << Form("bkg_norm2_2017 rateParam CR bkg_mass 1.0\n");
                  newcardShape << Form("bhh_norm_2017 rateParam * bhh_mass 1.0 [0.2,5]");
		}
		if (year[y] == "2018"){
		  newcardShape << Form("bkg_norm_2018 rateParam CatAB bkg_mass 1.0\n");
                  newcardShape << Form("bkg_norm2_2018 rateParam CR bkg_mass 1.0\n");
                  newcardShape << Form("bhh_norm_2018 rateParam * bhh_mass 1.0 [0.2,5]");
		}
