...
//   reader->AddVariable( "nmhits", &nmhits_c );
   ...
   TString fold[4]={"jpsi_mass3to4","jpsi0p015_sig","upsilon_mass9to10","upsilon0p03_PVd"};
   TString dir="/eos/user/w/wangz/darkphoton/BDTweight/2018/"+fold[type-1]+"/";
   if(year==2017) dir="/eos/user/w/wangz/darkphoton/BDTweight/2017/"+fold[type-1]+"/";
   TString methodName = "BDT method";
   TString weightfile = dir + TString("TMVAClassification_BDT.weights.xml");
   reader->BookMVA( methodName, weightfile );
}

void comment_mva(string treepath = "tree_1.root", int year=2018, bool isMC=false) {

   bool debug=false; 
   vector<string> filesToRun;
//   string dirIn="/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/";
   string dirIn="/eos/user/w/wangz/darkphoton/comment_mva_check/";
//   string dirIn="";
   ...
   for(unsigned int f=0; f<filesToRun.size(); f++){
    ...
    TBranch *mumva3 = tree->Branch("mva3",mva3,"mva3[4]/F");
    TBranch *mumva4 = tree->Branch("mva4",mva4,"mva4[4]/F");
...
//    entries=entries/10;
    for(int t=0; t<entries; t++){
      ...
      for (int i = 0; i < nmuon; i++) {
          ...
          mva3[i]=reader3->EvaluateMVA( "BDT method");
          mva4[i]=reader4->EvaluateMVA( "BDT method");
      }
     mumva->Fill();
     mumva2->Fill();
     mumva3->Fill();
