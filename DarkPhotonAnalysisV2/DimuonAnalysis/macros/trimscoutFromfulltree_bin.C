...
//   string dirIn="/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/";
   ...
   TTree* outtree = new TTree("tree", "tree");
    ...
    TH1D* mmpt[400];
    TH1D* massforLimit_CatA_bin1;
    TH1D* massforLimit_CatA_bin2;
    TH1D* massforLimit_CatA_bin3;
    TH1D* massforLimit_CatA_bin4;
    TH1D* massforLimit_CatB_bin1;
    TH1D* massforLimit_CatB_bin2;
    TH1D* massforLimit_CatB_bin3;
    TH1D* massforLimit_CatB_bin4;
    float m=0.2;
    ...
    for(int j=0; j<400; j++){
      ...
      if(j==249){
        massforLimit_CatA_bin1 = new TH1D(Form("massforLimit_CatA%d_pt30to35",j),Form("massforLimit_CatA%d_pt30to35",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatA_bin1->Sumw2();
        massforLimit_CatB_bin1 = new TH1D(Form("massforLimit_CatB%d_pt30to35",j),Form("massforLimit_CatB%d_pt30to35",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatB_bin1->Sumw2();
        massforLimit_CatA_bin2 = new TH1D(Form("massforLimit_CatA%d_pt35to40",j),Form("massforLimit_CatA%d_pt35to40",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatA_bin1->Sumw2();
        massforLimit_CatB_bin2 = new TH1D(Form("massforLimit_CatB%d_pt35to40",j),Form("massforLimit_CatB%d_pt35to40",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatB_bin1->Sumw2();
        massforLimit_CatA_bin3 = new TH1D(Form("massforLimit_CatA%d_pt40to50",j),Form("massforLimit_CatA%d_pt40to50",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatA_bin1->Sumw2();
        massforLimit_CatB_bin3 = new TH1D(Form("massforLimit_CatB%d_pt40to50",j),Form("massforLimit_CatB%d_pt40to50",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatB_bin1->Sumw2();
        massforLimit_CatA_bin4 = new TH1D(Form("massforLimit_CatA%d_pt50to100",j),Form("massforLimit_CatA%d_pt50to100",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatA_bin1->Sumw2();
        massforLimit_CatB_bin4 = new TH1D(Form("massforLimit_CatB%d_pt50to100",j),Form("massforLimit_CatB%d_pt50to100",j),100,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatB_bin1->Sumw2();
      } 
    }
   ...
   for(unsigned int f=0; f<filesToRun.size(); f++){
    ...
    while(reader.Next()) {
      ...
      if(*PVd>0.2) continue;
//      if(*PVd<0.015) continue;  //high PVd CR, no mva ID
...
//      if(*IP<3.5 || *IP>11) continue;    
      ...
      for (int i = 0; i < *nmuon; i++) {
          if(mupt[i]<5 || abs(mueta[i])>1.9) continue;
...
//        if(ptll<35) continue;
        //for pseudo signal, cut change
//        if(m2pt<5 || ptll<20) continue;
//          if(m2pt<5 || ptll<30) continue;
        if(mass<3.5 && (m2pt<5 || ptll<35)) continue;
        if(mass>=3.5 && (m2pt<5 || ptll<20)) continue;
...
//	if(!( l1Result->at(2) || l1Result->at(6) || l1Result->at(7) || l1Result->at(10) || l1Result->at(11) || l1Result->at(13))) continue;
	...
	for(int j=0; j<400.; j++){
			...
			if(m1ch*m2ch<0.  && m1pt>slidePt1 && m2pt>slidePt2 && maxEta>=1.2 && maxEta<2.4 ){ massforLimit_CatC[j]->Fill(mass,weight); }
               ...
               //add pt bin for special bin
               if(mass > ma-(ma*0.013*5.) && mass < ma+(ma*0.013*5.) && j==249) {
                        if(m1ch*m2ch<0.  && m1pt>slidePt1 && m2pt>slidePt2 && maxEta<0.9){ 
                             if(ptll>30 && ptll<35)  massforLimit_CatA_bin1->Fill(mass,weight); 
                             if(ptll>35 && ptll<40)  massforLimit_CatA_bin2->Fill(mass,weight);
                             if(ptll>40 && ptll<50)  massforLimit_CatA_bin3->Fill(mass,weight);
                             if(ptll>50 )  massforLimit_CatA_bin4->Fill(mass,weight);
                        }
                        if(m1ch*m2ch<0.  && m1pt>slidePt1 && m2pt>slidePt2 && maxEta>=0.9 && maxEta<1.9 ){ 
                             if(ptll>30 && ptll<35)  massforLimit_CatB_bin1->Fill(mass,weight);
                             if(ptll>35 && ptll<40)  massforLimit_CatB_bin2->Fill(mass,weight);
                             if(ptll>40 && ptll<50)  massforLimit_CatB_bin3->Fill(mass,weight);
                             if(ptll>50 )  massforLimit_CatB_bin4->Fill(mass,weight);
                        }                   
               }
