...
void trimscoutFromfulltree_noID(string treepath = "full_tem.root", const char* outfilename = "./scoutDataMVA_v2.root", bool isMC=false) {
...
//        if(mass<4 && (m2pt<5 || ptll<35)) continue;
//        if(mass>=4 && (m2pt<5 || ptll<20)) continue;

        if(mass<4 && *IP>3.5) continue;
        if(mass>4 && *PVd>0.015) continue;
