...
//   string dirIn="/eos/cms/store/group/phys_exotica/darkPhoton/jakob/newProd/";
   ...
   TTree* outtree = new TTree("tree", "tree");
    ...
    for(int j=0; j<400; j++){
      m = m+(m*0.01); 
      massforLimit_CatA[j] = new TH1D(Form("massforLimit_CatA%d",j),Form("massforLimit_CatA%d",j),1000,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatA[j]->Sumw2();
      massforLimit_CatB[j] = new TH1D(Form("massforLimit_CatB%d",j),Form("massforLimit_CatB%d",j),1000,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatB[j]->Sumw2();
      massforLimit_CatC[j] = new TH1D(Form("massforLimit_CatC%d",j),Form("massforLimit_CatC%d",j),1000,m-(m*0.013*5.),m+(m*0.013*5.));  massforLimit_CatC[j]->Sumw2();
...
//          if(muid[i]<0.5) continue;
	...
	float maxEta=TMath::Max(abs(m1eta),abs(m2eta));
        ...
        //mass smearing
        double random_v=distribution(generator);
        mass=mass*(1+random_v*smear_f*0.013);     
