...
void MVAread(TMVA::Reader *reader, int type=1){
   ...
   TString fold[10]={"JPsi0p2_noIP","JPsi0p2_IP","upsilon0p2","upsilon0p2_IP","upsilon0p2_PVd","upsilon0p05_PVd","upsilon0p03_PVd","mc0p2_noIP","mc0p2_IP","upsilon0p015_PVd"};
...
void MVAread(TMVA::Reader *reader, int type=1){
   ...
   if(type<5) reader->AddVariable( "ntklayers", &nlayer_c );
   if(type<4) reader->AddVariable( "chi", &chi_c );
   if(type<3) reader->AddVariable( "nmhits", &nmhits_c );
   if(type<2) reader->AddVariable( "nphits", &nphits_c );
   reader->AddVariable( "trkiso", &iso_c );
   reader->AddVariable( "vtxchi2",&vtxchi2_c);
   reader->AddVariable("PVd",&PVd_c);

   if(type<=0 || type>5){ cout << "INPUT TYPE BUG" << endl; return;}
   TString fold[5]={"upsilon0p03_no0","upsilon0p03_no1","upsilon0p03_no2","upsilon0p03_no3","upsilon0p03_no4"};
   TString dir="/afs/cern.ch/work/w/wangz/DarkPhotonAnalysisV2/MVA/darkphoton/diffvar/"+fold[type-1]+"/";
//   TString dir="/tmp/wangz/diffvar/"+fold[type-1]+"/";
...
void generate_mva(TString treepath = "2018A/tree_1.root", TString outfilename = "./scoutData4MuonsM70.root"){
...
//  entries=entries/10;
  ...
  for(int i=0; i<entries; i++){    
    tree->GetEntry(i);
//    if(i%10000==0) cout << " entries=" << i << endl;
    int indx1, indx2;
    float tempt=0;
    if(nmuon<2) continue;
    //pick first 2 muons
...
//   if(charge[indx1]*charge[indx2]<0) dr_n=fabs(temdr/mass/mean-1); 
   if(PVd<0.015){    
     ...
     m1mva5 = reader5->EvaluateMVA( "BDT method");
   }
   else{
     m1mva1=-1;
     m1mva2=-1;
...
//indx2
   ...
   if(PVd<0.015){
     ...
     m2mva5 = reader5->EvaluateMVA( "BDT method");
   }
   else{
     m2mva1=-1; 
     m2mva2=-1;
