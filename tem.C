...
void tem(string treepath = "scout_1.root", const char* outfilename = "./scoutData4MuonsM70.root", bool isMC=false) {
...
//        if(nmuon==2 && mupt[2]>0) cout  << "nm=" << idx  << " 4 muon id" <<muid[0]<<"," <<muid[1]<<"," << mupt[2]<<"," <<muid[3] << endl;
	...
	float BSx = 0.086;
	float BSy = -0.033;
        if(isMC){ BSx=0.01204; BSy=0.04107;}
...
//        if(!passPVconstraint) continue;
        if(nvtx<=0.) continue;
         count3++;
//        if(!(l1Result->at(4) || l1Result->at(11) || l1Result->at(12) || l1Result->at(10))) continue;
//         if(!(l1Result->at(2) ||l1Result->at(7) || l1Result->at(11) || l1Result->at(6))) continue;
	 count++;     	
         outtree->Fill();	
      }
   }
   cout << "2m=" << count2 << " before l1=" <<count3 <<  "\n final=" << count << endl;
   tof->cd();
